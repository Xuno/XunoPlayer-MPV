...
QTBASE=/home/lex/Qt/5.15.2/gcc_64
export LD_LIBRARY_PATH=${QTBASE}/lib
#export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${devtools}/opencv/opencv-4.2.0/build/lib
#export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${devtools}/ffmpeg/ffmpeg-build-script/workspace/lib
#export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${devtools}/im/ImageMagick-7.0.9-14/build/lib
#export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${devtools}/libraw/LibRaw-0.19.5/build/lib
#export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${devtools}/OpenImageIO/oiio/dist/lib
#IM_VER=ImageMagick-7.0.9
#IM_MODULES=${IM_VER}/modules-Q32HDRI

#MPVBUILD=201609151529-git-cf5c3fc-ffmpeg
#MPVPATH=${devtools}/mpv-player/mpv-build/build-${MPVBUILD}
#export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${MPVPATH}
...
# Copy all MPV  dependencies across to the tar directory.
...
#echo "Copying dependencies MPV..."
#
#for dep in `ldd ${MPVPATH}/libmpv.so | awk '{print $3}' | grep -v "("`
#do
#  cp $dep $libsdir
#  echo "Copied dependency "$dep" to "$libsdir
#done

echo "Copying dependencies more QT..."
libs="libQt5Svg"
t=$tardir/lib
for l in $libs; do
 s=${QTBASE}/lib/${l}.so.?
...
echo "copy shaders"
s=${PWD}/shaders/gather
t=$tardir/shaders/gather
mkdir -p $t
cp $s/*.hook $t

echo "copy translations"
s=${QTBASE}/translations
t=$tardir
mkdir -p $t
cp -r  $s $t


plugins="iconengines imageformats platformthemes xcbglintegrations sqldrivers position printsupport bearer"
qtttargetplugi=$tardir/plugins
for plug in $plugins; do
 s=${QTBASE}/plugins/${plug}
 t=${qtttargetplugi}/${plug}
 echo "copy plugin $t"
 mkdir -p $t
 cp ${s}/*.so  ${t}
done
...
qtplatformplugin=${QTBASE}/plugins/platforms/libqxcb.so
qtplatformplugindir=$tardir/plugins/platforms
...
echo "copy QtWebEngineProcess"
mkdir $tardir/libexec
cp ${QTBASE}/libexec/QtWebEngineProcess $tardir/libexec


echo "copy NSS for google on browser"
cp /usr/lib/x86_64-linux-gnu/nss/libsoftokn3.so  $tardir/lib
cp /usr/lib/x86_64-linux-gnu/nss/libnssckbi.so $tardir/lib
...
echo copy lib* from projetc 
cp $PWD/lib*.so.*  $libsdir

echo Purge common OpenGL libs
rm  $libsdir/libGL*.so.*
...
#echo copy mpv-prescaler
#mpvprescalerdir=$PWD/mpv-prescalers
#mpvprescalertardir=$tardir/mpv-prescalers
#mkdir $mpvprescalertardir
#cp $mpvprescalerdir/* $mpvprescalertardir


echo copy IM-coders
mpvprescalerdir=$PWD/lib/${IM_VER}
mpvprescalertardir=$libsdir
mkdir $mpvprescalertardir/${IM_VER}
cp -r $mpvprescalerdir $mpvprescalertardir



# Create the install dependede script.
execscript=$tardir/"install_dep.sh"
echo "Created run script: "$execscript
echo "#!/bin/sh" > $execscript
#echo "sudo add-apt-repository ppa:oibaf/graphics-drivers" >> $execscript
echo "sudo apt update" >> $execscript
echo "sudo apt upgrade" >> $execscript
echo "sudo apt install ocl-icd-opencl-dev mesa-opencl-icd" >> $execscript
echo "sudo apt install libva2 libva-drm2 libva-x11-2 libxcb-xinerama0" >> $execscript
...
echo "export LD_LIBRARY_PATH=\`pwd\`/lib" >> $execscript
echo "export QT_QPA_FONTDIR=\`pwd\`/fonts" >> $execscript
echo "export MAGICK_CODER_FILTER_PATH=\`pwd\`/lib/${IM_MODULES}/filters" >> $execscript
echo "export MAGICK_CODER_MODULE_PATH=\`pwd\`/lib/${IM_MODULES}/coders" >> $execscript
